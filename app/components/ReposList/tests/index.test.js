...
describe('<ReposList />', () => {
  ...
  it('should render the repositories if loading was successful', () => {
    ...
    const renderedComponent = shallow(
      <ReposList isevenResponse={isevenResponse} error={false} />
    );

    expect(
      renderedComponent.contains(
        <List items={isevenResponse} component={RepoListItem} />
      )
    ).toEqual(true);
  });
  ...
  it('should not render anything if nothing interesting is provided', () => {
    const renderedComponent = shallow(
      <ReposList isevenResponse={false} error={false} loading={false} />
    );
